...
> ℹ️ **关于本章对照表的说明**：下面的"架构对比"与"文件映射"用于帮助读者建立 **"我们这份 ~4300 行最小实现"** 与 **"Claude Code 这类生产级 Coding Agent"** 在概念上的对应关系。
> 本项目是**独立的 clean-room 教育实现**——代码并非来自 Claude Code 的内部实现，所有对照仅用于学习与扩展研究的类比目的。具体细节可能与 Claude Code 真实内部实现存在差异。
...
`/goal`、`/loop`、Auto Mode 三件套也补上了，细节在 [第 15 章：自治与续跑](docs/15-autonomy.md)。它们让 agent 能跨很多 turn、在无人盯着的情况下继续往前走：`/goal` 用一个独立评估器把停止条件追到达成或判定不可能，`/loop` 让主模型按间隔或自定节奏排下一次运行，Auto Mode 用一个读脱敏对话记录的分类器代替危险操作的确认框。提示词逐字取自泄露实锤，机制上做了几处教学简化（单段分类器、会话内定时器代替 KAIROS）——本章下面「扩展方向」里原本对这类能力的设想，可以对照第 15 章看已经落地到什么程度。
...
~5400 行代码（TS）/ ~5000 行（Python），12 个文件，覆盖了一个 coding agent 的核心组件、进阶能力和自主运行：
